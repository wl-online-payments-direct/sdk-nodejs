...
describe("Payment products", () => {
  ...
  describe("Callback", () => {
    test("it can get payment products", done => {
      sdkInstance.products
        .getPaymentProducts(config.merchantId, {
          countryCode: "NL",
          currencyCode: "EUR"
        })
        .then(sdkResponse => {
    ...
    test("it can get a payment product", done => {
      sdkInstance.products
        .getPaymentProduct(config.merchantId, PAYMENT_PRODUCT_ID, {
          countryCode: "NL",
          currencyCode: "EUR"
        })
        .then(sdkResponse => {
    ...
    test("it can get payment product networks", done => {
      sdkInstance.products
        .getPaymentProductNetworks(config.merchantId, PAYMENT_PRODUCT_ID, {
          countryCode: "NL",
          currencyCode: "EUR"
        })
        .then(sdkResponse => {
    ...
    test("it can get a product directory", done => {
      sdkInstance.products
        .getProductDirectory(config.merchantId, PAYMENT_PRODUCT_ID, {
          countryCode: "NL",
          currencyCode: "EUR"
        })
        .then(sdkResponse => {
